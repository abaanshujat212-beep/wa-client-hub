(function (root, factory) {
  ...
  function createSignupCoordinator(options) {
    ...
    var cancel = typeof options.cancel === "function" ? options.cancel : function () { return Promise.resolve(); };
    ...
    var attempt = null;
    function active(state) { return Boolean(attempt) && (state === undefined || attempt.state === state); }
    ...
    function receiveLoginResult(result) {
      ...
      if (result && result.authResponse && result.authResponse.code) return receiveCode(result.authResponse.code);
      // An empty SDK callback is not evidence that the popup has closed.
      log("signup.authorization.pending", { hasState: true, hasCode: Boolean(attempt.code) });
      emit("pending", "Complete the Meta popup. If you closed it, use Cancel here before trying again.");
    ...
    return { start: start, active: active, receiveLoginResult: receiveLoginResult, receiveCode: receiveCode, receiveFinish: receiveFinish, receiveCancel: receiveCancel, receiveError: receiveError, popupClosed: popupClosed, finalizePopupClose: finalizePopupClose, finalizeReconciliation: finalizeReconciliation, explicitCancel: explicitCancel, fail: fail };
  }
  function signupErrorMessage(error) {
    if (error && error.status === 429) return "Too many signup attempts. Wait " + Math.max(1, Math.ceil((error.retryAfter || 600) / 60)) + " minute(s), then try again.";
    if (error && error.status === 401) return "Your session expired. Sign in again, then connect with Meta.";
    if (error && error.status === 403) return "Your security token expired or the request origin is invalid. Refresh this page and try again.";
    if (error && error.status === 400) return "Choose a workspace and enter a number label of at least 2 characters.";
    if (error && error.status === 404) return "Meta signup or the selected workspace is unavailable. Refresh this page.";
    if (error && error.status >= 500) return "Meta signup is temporarily unavailable on the server. Please try again later.";
    if (error && error.code === "META_SDK_LOAD_FAILED") return "Facebook could not load. Check your connection or browser blocking, then try again.";
    return "Could not start Meta signup. Check your connection and allow Facebook popups, then try again.";
  }
  function mount() {
    ...
    async function json(url, options) { var response = await fetch(url, Object.assign({ credentials: "same-origin" }, options || {})); var body = response.status === 204 ? null : await response.json().catch(function () { return {}; }); if (!response.ok) { var error = new Error(body.error || "Signup request failed"); error.status = response.status; error.retryAfter = Number(response.headers.get("Retry-After")) || 0; throw error; } return body; }
    ...
    async function refresh() { if (!select.value) return; try { var result = await json("/api/meta/signup/status?workspaceId=" + encodeURIComponent(select.value)); status.textContent = result.pending ? "Signup awaiting completion." : result.connections.length ? result.connections.map(function (c) { return c.label + " — " + (c.status === "connecting" ? "Awaiting connection checks and activation" : c.status) + (c.number ? " — " + c.number.phone : ""); }).join("\n") : "No official Meta connection yet."; } catch (_) { status.textContent = "Status unavailable."; } }
    function onCoordinatorStatus(update) { say(update.message); if (update.terminal) { resetControls(); void refresh(); window.dispatchEvent(new Event("meta-connections-updated")); } }
    ...
    function loadSdk() { return new Promise(function (resolve, reject) { if (root.FB) return resolve(); root.fbAsyncInit = function () { root.FB.init({ appId: config.appId, autoLogAppEvents: true, xfbml: false, version: config.graphVersion }); resolve(); }; if (document.getElementById("facebook-jssdk")) return; var script = document.createElement("script"); script.id = "facebook-jssdk"; script.async = true; script.defer = true; script.crossOrigin = "anonymous"; script.src = "https://connect.facebook.net/en_US/sdk.js"; script.onerror = function () { script.remove(); var error = new Error("Could not load Meta signup."); error.code = "META_SDK_LOAD_FAILED"; reject(error); }; document.head.appendChild(script); }); }
    button.addEventListener("click", async function () { if (coordinator.active() || button.disabled) return; try { button.disabled = true; var started = await post("start", { workspaceId: select.value, label: panel.querySelector("#metaSignupLabel").value.trim() }); coordinator.start(started.state); cancelButton.classList.remove("hidden"); say("Complete the Meta popup."); await loadSdk(); root.FB.login(function (result) { diagnostic("fb.login.callback", { hasState: coordinator.active(), hasAuthResponse: Boolean(result && result.authResponse), hasCode: Boolean(result && result.authResponse && result.authResponse.code), status: safeText(result && result.status, 80), authStatus: safeText(result && result.authResponse && result.authResponse.status, 80) }); if (!coordinator.active(started.state)) return; void coordinator.receiveLoginResult(result); }, { config_id: config.configId, auth_type: "rerequest", response_type: "code", override_default_response_type: true, extras: { setup: {} } }); } catch (error) { var message = signupErrorMessage(error); diagnostic("signup.start.failed", { status: error.status || 0, reason: error.code || "START_REQUEST_FAILED" }); if (coordinator.active()) void coordinator.fail(message, "SDK_START_FAILED", true); else { resetControls(); say(message); } } });
    ...
    async function discover() { try { var session = await json("/api/session"); if (!session.authenticated) return; csrf = session.csrfToken; config = await json("/api/meta/signup/config").catch(function () { return null; }); button.disabled = !config; var workspaces = (await json("/api/workspaces")).workspaces || []; if (root.WA_EMBEDDED_WORKSPACE) workspaces = workspaces.filter(function (w) { return w.id === root.WA_EMBEDDED_WORKSPACE; }); var previous = select.value; select.replaceChildren(); workspaces.forEach(function (workspace) { var option = document.createElement("option"); option.value = workspace.id; option.textContent = workspace.name; select.appendChild(option); }); if (workspaces.some(function(w) { return w.id === previous; })) select.value = previous; if (!workspaces.length) { panel.classList.add("hidden"); return; } panel.classList.remove("hidden"); await refresh(); } catch (_) { panel.classList.add("hidden"); } }
    window.addEventListener("workspaces-updated", discover);
    var app = document.querySelector("#appView"); if (app) { new MutationObserver(function () { if (!app.classList.contains("hidden")) void discover(); }).observe(app, { attributes: true, attributeFilter: ["class"] }); if (!app.classList.contains("hidden")) void discover(); }
  }
  return { signupErrorMessage: signupErrorMessage, parseMessage: parseMessage, createSignupCoordinator: createSignupCoordinator, mount: mount, FACEBOOK_ORIGINS: ORIGINS };
});
