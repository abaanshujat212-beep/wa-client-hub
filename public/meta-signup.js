(function (root, factory) {
  ...
  function mount() {
    ...
    async function json(url, options) { var response = await fetch(url, Object.assign({ credentials: "same-origin" }, options || {})); var body = response.status === 204 ? null : await response.json().catch(function () { return {}; }); if (!response.ok) { var error = new Error("Signup request failed"); error.status = response.status; error.retryAfter = Number(response.headers.get("Retry-After")) || 0; throw error; } return body; }
    ...
    async function discover() { try { var session = await json("/api/session"); if (!session.authenticated) return; csrf = session.csrfToken; config = await json("/api/meta/signup/config"); var workspaces = (await json("/api/workspaces")).workspaces || []; if (root.WA_EMBEDDED_WORKSPACE) workspaces = workspaces.filter(function (w) { return w.id === root.WA_EMBEDDED_WORKSPACE; }); select.replaceChildren(); workspaces.forEach(function (workspace) { var option = document.createElement("option"); option.value = workspace.id; option.textContent = workspace.name; select.appendChild(option); }); if (!workspaces.length) return; panel.classList.remove("hidden"); await refresh(); } catch (_) { panel.classList.add("hidden"); } }
