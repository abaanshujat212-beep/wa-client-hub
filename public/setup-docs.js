(() => {
  ...
  const rows = [
    ['CRM OAuth callback', `${publicOrigin}/oauth/crm/callback`, 'Marketplace App → Auth → Redirect URLs'],
    ['CRM app events', `${publicOrigin}/webhooks/ghl/events`, 'Marketplace App → Webhooks'],
    ['CRM Conversation Provider delivery', `${publicOrigin}/webhooks/ghl/messages`, 'Conversation Provider → SMS → Delivery URL'],
  ...
  function copy(value, button) { navigator.clipboard.writeText(value).then(() => { const old = button.textContent; button.textContent = 'Copied'; setTimeout(() => { button.textContent = old; }, 1200); }).catch(() => { button.textContent = 'Copy manually'; }); }
  function install() {
    ...
    view.innerHTML = `<div class="hero-card"><div><span class="kicker">Deployment runbook</span><h3>OAuth, webhooks and provider setup.</h3><p>Copy the exact permanent Cloudflare URLs. Never put client secrets in a browser or Git.</p></div><a class="button light" href="https://marketplace.gohighlevel.com" target="_blank" rel="noreferrer">Open Marketplace</a></div>
      <div class="setup-grid">
        <article class="setup-card wide-card"><h3>Public endpoint directory</h3><p class="muted">Base URL: <code>${publicOrigin}</code>. Cloudflare must forward this hostname to <code>http://localhost:3131</code>. Every provider URL must remain HTTPS and unchanged.</p><div class="endpoint-list">${rows.map(([name,url,where]) => `<div class="endpoint-row"><div><strong>${escapeHtml(name)}</strong><small>${escapeHtml(where)}</small><code>${escapeHtml(url)}</code></div><button class="button secondary" data-copy="${escapeHtml(url)}">Copy</button></div>`).join('')}</div></article>
        <article class="setup-card"><h3>1. CRM OAuth ENV</h3><p>Put these values in local <code>.env</code>, then restart the app.</p><pre>APP_ORIGIN=${publicOrigin}
COOKIE_SECURE=true
GHL_CLIENT_ID=&lt;Marketplace Client ID&gt;
GHL_CLIENT_SECRET=&lt;Marketplace Client Secret&gt;
GHL_REDIRECT_URI=${publicOrigin}/oauth/crm/callback
...
GHL_REQUIRED_SCOPES=conversations.write</pre><button class="button secondary" data-copy="APP_ORIGIN=${publicOrigin}\nCOOKIE_SECURE=true\nGHL_REDIRECT_URI=${publicOrigin}/oauth/crm/callback\nGHL_AUTH_URL=https://marketplace.gohighlevel.com/oauth/chooselocation\nGHL_TOKEN_URL=https://services.leadconnectorhq.com/oauth/token\nGHL_API_BASE_URL=https://services.leadconnectorhq.com\nGHL_API_VERSION=2023-02-21\nGHL_REQUIRED_SCOPES=conversations.write">Copy non-secret ENV</button></article>
        <article class="setup-card"><h3>2. Marketplace app</h3><ol><li>Create a <strong>Private</strong> Marketplace app.</li><li>Enter the Base URL and the exact neutral CRM OAuth callback above.</li><li>Request only <code>conversations.write</code>.</li><li>Do not select <code>conversations.read</code>, <code>contacts.read</code>, or <code>locations.read</code>.</li><li>Configure both distinct webhook URLs shown above.</li><li>Record the issued <code>conversationProviderId</code>.</li></ol><p class="warning-text">The client secret is shown once. Store it locally; do not paste it into this page.</p></article>
        <article class="setup-card"><h3>3. Exact mapping</h3><p>After OAuth, an operator must bind one company/location to one workspace, one WhatsApp number, one active provider connection, and the issued <code>conversationProviderId</code>. Ambiguous or missing mappings fail closed.</p><p>Use <strong>Integrations → CRM → Connect / reconnect CRM</strong>, then review the readiness panel before sending.</p></article>
        <article class="setup-card"><h3>4. Runtime requirements</h3><pre>STORE_DRIVER=postgres
DATABASE_URL=postgresql://...
REDIS_URL=redis://...
CONNECTOR_MASTER_KEY=&lt;base64 32-byte key&gt;</pre><p>Run <code>npm run db:migrate</code>, restart with <code>npm start</code>, sign in, open <strong>Integrations</strong>, and review CRM readiness.</p></article>
        <article class="setup-card"><h3>5. Cloudflare check</h3><ul><li>DNS hostname: <code>wa.10xcollab.com</code></li><li>Origin service: <code>http://localhost:3131</code></li><li>SSL mode: HTTPS externally</li><li>Do not cache <code>/api/*</code>, <code>/oauth/*</code>, or <code>/webhooks/*</code></li></ul><a href="${publicOrigin}/api/health" target="_blank" rel="noreferrer">Open public health check ↗</a></article>
        <article class="setup-card wide-card"><h3>Manual owner checklist</h3><ul><li>Provision the permanent named tunnel and verify the public origin.</li><li>Enter the exact neutral callback and webhook URLs in Marketplace.</li><li>Keep provider credentials in the deployment secret store.</li><li>Complete OAuth and mapping in the UI; do not edit the database directly.</li><li>Run one controlled outbound/inbound message test and inspect diagnostics.</li></ul></article>
