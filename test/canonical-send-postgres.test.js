...
test("PostgreSQL canonical send reserves concurrent idempotency keys before dispatch", { skip: !connectionString }, async () => {
  ...
  const postgres = new PostgresRepository({ pool, connectionString });
  const store = new PostgresStore(process.cwd(), { repository: postgres });
  const suffix = crypto.randomUUID();
  try {
    await postgres.init();
    await store.init({ adminEmail: "admin@test.local", adminPassword: "AdminPassword123!" });
    ...
    await openwa.enable({ workspaceId: workspace.id, numberId: number.id, sessionId: `session-${suffix}`, label: number.label, riskAcknowledgedBy: user.id });
    ...
    let providerCalls = 0;
    const service = new CanonicalSendService({
      repository: new MessagingRepository(pool),
      adapters: { openwa: { async sendText() { providerCalls += 1; await new Promise((resolve) => setTimeout(resolve, 75)); return { externalMessageId: `external-${suffix}`, rawStatus: "queued" }; } } }
    });
    ...
    assert.equal(results.filter((result) => !result.duplicate).length, 1);
    const messages = await pool.query("SELECT id FROM messages WHERE workspace_id=$1 AND client_idempotency_key=$2", [workspace.id, input.idempotencyKey]);
    assert.equal(messages.rowCount, 1);
    const attempts = await pool.query("SELECT status,message_id FROM message_send_attempts WHERE workspace_id=$1 AND client_idempotency_key=$2", [workspace.id, input.idempotencyKey]);
    assert.equal(attempts.rowCount, 1);
    assert.equal(attempts.rows[0].status, "accepted");
    assert.equal(attempts.rows[0].message_id, messages.rows[0].id);
