...
const {
  ...
  normalizeText
} = require("../src/messaging/canonicalSendService");
...
function fixture(overrides = {}) {
  ...
  const repository = {
    ...
    async outboundByIdempotencyKey() { return null; },
    async recordOutbound(input) { calls.push(["record", input]); return { id: "message-a", status: "accepted" }; },
    ...overrides.repository
  };
  const adapters = {
    openwa: {
      async sendText(input) { calls.push(["send", input]); return { externalMessageId: "provider-a", rawStatus: "queued" }; }
    },
    ...overrides.adapters
  };
  ...
  return {
    calls,
    service: new CanonicalSendService({ repository, adapters, events, audit })
  };
}
...
test("canonical send returns an existing idempotent result without dispatch", async () => {
  const { service, calls } = fixture({
    repository: {
      async outboundByIdempotencyKey() {
        return { id: "existing", status: "accepted" };
      }
    }
  });
...
test("canonical send fails closed when a provider adapter is unavailable", async () => {
  ...
  await assert.rejects(
    ...
    (error) => error instanceof SendError && error.code === "PROVIDER_UNAVAILABLE" && error.status === 409
  );
});
...
test("OpenWA adapter converts E.164 recipients without exposing credentials", async () => {
  const sent = [];
  const adapter = new OpenWaMessagingAdapter({
    async sendText(to, text) { sent.push({ to, text }); return "openwa-message"; }
  });
  const result = await adapter.sendText({
    connection: { provider: "openwa" },
    to: "+92 300 111-2222",
    text: "Hello"
  });
