...
function isValidPhone(phone) {
  return /^\+?[0-9][0-9\s().-]{7,20}$/.test(String(phone || "").trim());
}
...
app.post("/api/users", requireAuth, requireAdmin, async (req, res) => {
  ...
  const password = String(req.body.password || "");
  if (name.length < 2 || !isValidEmail(email) || password.length < 10) {
...
app.post("/api/me/password", requireAuth, async (req, res) => {
  ...
  const changed = await store.changePassword(req.user.id, currentPassword, newPassword);
  if (!changed) return res.status(400).json({ error: "Current password is incorrect" });
  store.addAudit(req.user.id, "password.changed");
  res.json({ ok: true });
});
...
app.post("/api/users/:id/password", requireAuth, requireAdmin, async (req, res) => {
  const newPassword = String(req.body.newPassword || "");
  if (newPassword.length < 10) return res.status(400).json({ error: "New password must be at least 10 characters" });
  const user = await store.resetPassword(req.params.id, newPassword);
  if (!user) return res.status(404).json({ error: "Client not found" });
  store.addAudit(req.user.id, "client.password.reset", { clientId: user.id });
  res.json({ user });
});
...
app.post("/api/accounts", requireAuth, (req, res) => {
  ...
  if (label.length < 2 || !isValidPhone(phone)) return res.status(400).json({ error: "Enter an account name and phone number" });
  try {
...
app.delete("/api/accounts/:id", requireAuth, async (req, res) => {
  ...
  await launcher.removeProfile(account);
  const deleted = store.deleteAccount(account.id);
  store.addAudit(req.user.id, "account.deleted", { accountId: account.id, ownerId: account.ownerId });
  res.json({ ok: true, account: deleted });
});
