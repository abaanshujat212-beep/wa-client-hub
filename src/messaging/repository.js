...
class MessagingRepository {
  ...
  async reserveOutbound({ dispatch, idempotencyKey, requestHash }) {
    ...
    const existing = await this.pool.query(`SELECT a.*,m.external_message_id,m.status AS message_status FROM message_send_attempts a LEFT JOIN messages m ON m.id=a.message_id AND m.workspace_id=a.workspace_id WHERE a.workspace_id=$1 AND a.client_idempotency_key=$2`, [dispatch.workspaceId, idempotencyKey]);
