...
class MessagingRepository {
  ...
  async recordOutbound({ attemptId, dispatch, body, type, origin, externalMessageId, idempotencyKey, rawProviderStatus }) {
    const client = await this.pool.connect(); try { await client.query("BEGIN"); const id = crypto.randomUUID();
      ...
      const message = inserted.rows[0] || (await client.query("SELECT * FROM messages WHERE workspace_id=$1 AND client_idempotency_key=$2", [dispatch.workspaceId, idempotencyKey])).rows[0]; if (!message) throw new Error("Canonical outbound message could not be resolved");
