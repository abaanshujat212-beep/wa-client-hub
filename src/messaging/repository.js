...
class MessagingRepository {
  ...
  async recordOutbound({ attemptId, dispatch, body, type, origin, externalMessageId, idempotencyKey, rawProviderStatus, attachment = null }) {
    ...
    try {
      ...
      if (attachment) {
        await client.query(`INSERT INTO message_attachments(id,workspace_id,message_id,provider_connection_id,provider_media_id,media_type,file_name,size_bytes,sha256) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (provider_connection_id,provider_media_id) WHERE provider_connection_id IS NOT NULL AND provider_media_id IS NOT NULL DO NOTHING`, [crypto.randomUUID(), dispatch.workspaceId, message.id, dispatch.providerConnectionId, attachment.mediaId, attachment.type, attachment.filename, attachment.sizeBytes, attachment.sha256]);
      }
