...
const MULTIPART_LIMITS = Object.freeze({ maxBytes: 110 * 1024 * 1024, maxPartBytes: 100 * 1024 * 1024 });
...
function mapError(error) {
  ...
  if (code === 'META_MEDIA_MULTIPART_TOO_LARGE' || code === 'META_MEDIA_MULTIPART_PART_TOO_LARGE' || code === 'META_MEDIA_MULTIPART_HEADERS_TOO_LARGE') return { status: 413, body: { error: 'Meta media request is too large', code: 'META_MEDIA_REQUEST_TOO_LARGE' } };
...
function validMultipartUpload(body) { return Boolean(body && typeof body === 'object' && !Array.isArray(body) && Object.keys(body).length === 5 && validId(body.workspaceId) && validId(body.numberId) && typeof body.mimeType === 'string' && typeof body.filename === 'string' && Buffer.isBuffer(body.bytes)); }
