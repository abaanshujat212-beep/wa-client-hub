...
function createMetaSignupRouter({ enabled = false, pool, signupService, vault, origin } = {}) {
  ...
  const handlers = createMetaSignupHandlers({ authorization: new NumberCreationPolicy(pool), stateRepository: new MetaSignupStateRepository(pool), signupService, connectionRepository: new MetaConnectionRepository(pool, vault) });
  const guard = action => async (req, res, next) => {
    if (typeof req.session?.userId !== 'string' || !req.session.userId || req.session.userId.length > 256 || typeof req.sessionID !== 'string' || !req.sessionID || req.sessionID.length > 256) return res.status(401).json({ error: 'Please sign in' });
    ...
    try { const actor = (await pool.query('SELECT id,active FROM users WHERE id=$1 AND active=true', [req.session.userId])).rows[0]; if (!actor) return res.status(401).json({ error: 'Please sign in' }); req.user = actor; const admission = await protection.consume(actor.id, action); if (!admission.allowed) return res.set('Retry-After', String(admission.retryAfter)).status(429).json({ error: 'Too many signup attempts', code: 'META_SIGNUP_RATE_LIMITED' }); if (!req.is('application/json')) return res.status(415).json({ error: 'JSON body required' }); next(); }
    catch { return res.status(503).json({ error: 'Meta signup is temporarily unavailable', code: 'META_SIGNUP_UNAVAILABLE' }); }
  };
  for (const action of ['start', 'complete']) router.post(`/${action}`, guard(action), express.json({ limit: '16kb', strict: true }), (req, res, next) => { if (Buffer.byteLength(JSON.stringify(req.body || {})) > 16384) return res.status(413).json({ error: 'Signup request is too large' }); if (!validBody(action, req.body)) return res.status(400).json({ error: 'Valid signup details are required' }); return handlers[action](req, res, next); });
