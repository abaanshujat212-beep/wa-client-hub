...
function normalizeInstall(input = {}) {
  ...
  if (label.length < 2 || !/^\+\d{8,15}$/.test(phone) || !/^\d+$/.test(phoneNumberId) || !/^\d+$/.test(businessAccountId) || accessToken.length < 20) {
    throw new MetaConnectionError("Valid Meta connection details are required", "META_INSTALL_INVALID");
  }
  return { label, phone, phoneNumberId, businessAccountId, accessToken };
}
class MetaConnectionRepository {
  constructor(pool, vault) {
    if (!pool) throw new TypeError("pool is required");
    if (!vault) throw new TypeError("vault is required");
    this.pool = pool; this.vault = vault;
  }
  async install(input) {
    const value = normalizeInstall(input);
    const connectionId = crypto.randomUUID(); const numberId = crypto.randomUUID();
    ...
    try {
      ...
      await client.query(
        ...
        [connectionId, input.workspaceId, value.label, encrypted.ciphertext, encrypted.keyId, { phoneNumberId: value.phoneNumberId, businessAccountId: value.businessAccountId }],
      );
      await client.query(
      ...
      if (input.actorId) await client.query(
        "INSERT INTO audit_logs(id,user_id,action,details) VALUES($1,$2,'meta.connection.installed',$3)",
        [crypto.randomUUID(), input.actorId, { workspaceId: input.workspaceId, connectionId, numberId }],
      );
      await client.query("COMMIT");
      return {
        connection: { id: connectionId, workspaceId: input.workspaceId, provider: "whatsapp_cloud", label: value.label, status: "connecting", hasCredentials: true, phoneNumberId: value.phoneNumberId, businessAccountId: value.businessAccountId },
        number: { id: numberId, workspaceId: input.workspaceId, label: value.label, phone: value.phone, providerConnectionId: connectionId, automationEnabled: false },
      };
    } catch (error) {
