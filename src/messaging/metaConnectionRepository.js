...
function normalizeInstall(input = {}) {
  ...
  const businessPortfolioId = input.businessPortfolioId == null ? null : String(input.businessPortfolioId).trim(); const metaUserId = input.metaUserId == null ? null : String(input.metaUserId).trim(); const accessToken = String(input.accessToken || '').trim();
  if (label.length < 2 || !/^\+\d{8,15}$/.test(phone) || !/^\d+$/.test(phoneNumberId) || !/^\d+$/.test(businessAccountId) || (businessPortfolioId && !/^\d+$/.test(businessPortfolioId)) || (metaUserId && !/^\d+$/.test(metaUserId)) || accessToken.length < 20) throw new MetaConnectionError('Valid Meta connection details are required', 'META_INSTALL_INVALID');
  return { label, phone, phoneNumberId, businessAccountId, businessPortfolioId, metaUserId: metaUserId || null, accessToken, verifiedName: String(input.verifiedName || '').trim() || null, expiresIn: Number.isFinite(Number(input.expiresIn)) && Number(input.expiresIn) > 0 ? Number(input.expiresIn) : null };
}
class MetaConnectionRepository {
  ...
  async install(input) {
    ...
    try {
      ...
      await client.query(`INSERT INTO meta_connection_assets(provider_connection_id,workspace_id,meta_user_id,business_portfolio_id,waba_id,phone_number_id,display_phone_number,verified_name,token_type,token_expires_at,token_status) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'business',CASE WHEN $9::double precision IS NULL THEN NULL ELSE clock_timestamp()+($9::double precision*interval '1 second') END,'unverified')`, [connectionId, input.workspaceId, value.metaUserId, value.businessPortfolioId, value.businessAccountId, value.phoneNumberId, value.phone, value.verifiedName, value.expiresIn]);
      ...
      if (input.coexistence === true) {
        await client.query("UPDATE provider_connections SET settings=settings||'{\"coexistence\":true}'::jsonb WHERE id=$1", [connectionId]);
        await client.query("INSERT INTO meta_coexistence_jobs(provider_connection_id,step) VALUES($1,'subscribe'),($1,'smb_app_state_sync'),($1,'history')", [connectionId]);
      }
