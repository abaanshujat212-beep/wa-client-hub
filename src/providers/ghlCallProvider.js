...
function normalizeCallEvent(body = {}) {
  ...
  const conversationProviderId = String(body.conversationProviderId || body.conversation_provider_id || '').trim();
  ...
  const mediaKind = String(body.mediaKind || body.media_kind || 'voice').toLowerCase();
  if (!locationId || !installationId || !conversationProviderId || !callId || !eventId) throw new Error('GHL call event requires installationId, locationId, conversationProviderId, callId, and eventId');
  if (!['incoming', 'outgoing'].includes(direction)) throw new Error('GHL call direction is invalid');
  if (mediaKind !== 'voice') throw new Error('Only audio Calling events are supported');
  return {
    locationId,
    installationId,
  ...
  router.post('/', express.raw({ type: 'application/json', limit: '256kb' }), async (req, res) => {
    try {
      ...
      const mapping = await pool.query(`SELECT m.whatsapp_number_id,m.provider_connection_id,m.workspace_id,i.id AS installation_row_id,i.installation_id,i.location_id
        ...
        WHERE i.installation_id=$1 AND i.location_id=$2 AND m.conversation_provider_id=$3`, [call.installationId, call.locationId, call.conversationProviderId]);
      if (mapping.rowCount !== 1) return res.status(409).json({ error: 'Exact GHL location and conversation-provider mapping is required', code: 'GHL_CALL_MAPPING_REQUIRED' });
      ...
      const inserted = await pool.query(`INSERT INTO call_events(id,workspace_id,whatsapp_number_id,provider_connection_id,external_call_id,external_event_id,direction,state,media_kind,occurred_at,details)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,clock_timestamp(),$10) ON CONFLICT DO NOTHING RETURNING id`,
      [crypto.randomUUID(), row.workspace_id, row.whatsapp_number_id, row.provider_connection_id, call.callId, call.eventId, call.direction, call.state, call.mediaKind, { source: 'ghl', installationId: call.installationId, locationId: call.locationId, conversationProviderId: call.conversationProviderId, phone: call.phone, durationSeconds: call.durationSeconds, recordingUrl: call.recordingUrl, raw: call.raw }]);
      return res.status(inserted.rowCount ? 202 : 200).json({ accepted: true, duplicate: !inserted.rowCount, callId: call.callId });
    } catch (error) {
      const message = error.message === 'GHL call event requires installationId, locationId, conversationProviderId, callId, and eventId' || error.message === 'Only audio Calling events are supported' ? error.message : 'Invalid GHL call event';
