...
function Run-Compose([string[]]$Arguments) { & docker compose @Arguments; if ($LASTEXITCODE -ne 0) { throw "Docker Compose failed with exit code $LASTEXITCODE." } }
function Sync-PostgresPassword([string]$ComposeFile) { & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot 'sync-postgres-role-password.ps1') -EnvFile '.env.docker' -ComposeFile $ComposeFile; if ($LASTEXITCODE -ne 0) { throw 'PostgreSQL password synchronization failed.' } }
...
Run-Compose @('--env-file', '.env.docker', '-f', 'compose.infra.yml', 'up', '-d', 'postgres', 'redis', '--wait')
Sync-PostgresPassword 'compose.infra.yml'
& npm run db:migrate
